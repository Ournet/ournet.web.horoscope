...
route.get('/' + utils.WEEKLY_ROUTE, function(req, res, next) {
	...
	res.locals.site.head.canonical = 'http://' + config.host + links.horoscope.sign(config.weeklyName);
...
route.get('/:signSlug(\\w+)/' + utils.WEEKLY_ROUTE, function(req, res, next) {
	...
	res.locals.site.head.canonical = 'http://' + config.host + links.horoscope.sign.period(sign.slug, config.weeklyName);
