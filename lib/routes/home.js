...
route.get('/:signId(\\d{1,2})', function (req, res) {
	const config = res.locals.config;
	const culture = res.locals.currentCulture;

	utils.maxageRedirect(res);

	const sign = Data.signName(req.params.signId, culture.language);
	const links = res.locals.links;

	res.redirect(301, links.horoscope.sign(sign.slug, { ul: culture.language }));
});
